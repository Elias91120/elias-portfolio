...
function ChatPanel({
  ...
  const { messages, busy, streaming, ask } = useAgentChat();
  ...
  const panelClass = isMobile
    ? "ask-widget fixed inset-x-0 bottom-0 z-[70] flex max-h-[min(92svh,100dvh)] flex-col overflow-hidden rounded-t-3xl bg-[#120e20]/98 backdrop-blur-xl ring-1 ring-white/12 shadow-[0_-16px_64px_-12px_rgba(0,0,0,0.8)]"
    : "ask-widget fixed bottom-20 right-5 z-[70] flex max-h-[min(34rem,calc(100svh-7rem))] w-[min(24rem,calc(100vw-2.5rem))] flex-col overflow-hidden rounded-3xl bg-[#120e20]/95 backdrop-blur-xl ring-1 ring-white/12 shadow-[0_32px_80px_-24px_rgba(0,0,0,0.8)]";

  return (
    <motion.div
      initial={isMobile ? { opacity: 0, y: "100%" } : { opacity: 0, y: 24, scale: 0.96 }}
      animate={isMobile ? { opacity: 1, y: 0 } : { opacity: 1, y: 0, scale: 1 }}
      exit={isMobile ? { opacity: 0, y: "100%" } : { opacity: 0, y: 24, scale: 0.96 }}
    ...
    >
      ...
      >
        ...
        {messages.map((m, i) => {
          ...
          const isTyping = busy && i === messages.length - 1 && !m.content;
      ...
      <form
        ...
        className={`flex items-center gap-2 border-t border-white/8 bg-white/[0.02] px-4 py-3.5 ${
          isMobile ? "ask-widget-sheet-form" : ""
        }`}
      >
        <input
          value={input}
...
export default function AskWidget() {
  const { panelOpen, setPanelOpen, hasUnread } = useAgentChat();
    ...
    <>
      {showFab && (
        <motion.button
          ...
          className="ask-widget ask-widget-fab fixed bottom-5 right-5 z-[70] flex items-center gap-2.5 rounded-full bg-[#181230] px-4 py-3 text-sm font-medium text-white ring-1 ring-accent/40 shadow-[0_8px_40px_-8px_rgba(167,139,250,0.45)] transition-all duration-300 hover:ring-accent/70 hover:shadow-[0_8px_48px_-6px_rgba(167,139,250,0.6)] cursor-pointer"
          aria-label={
            panelOpen
